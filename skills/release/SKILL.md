...
After a release is cut on the hosted platform, the worker checks its **readiness** for every scope of the app (configuration, manifests, credentials, permissions, the destination's state — nothing is built). A release cut from a `hotfix/*` branch is a hotfix and may go to any scope; an `-rc.N` version is a candidate and stops at `low`.
