...
4. `init_project` (hosted: `init_app`, which also needs the `project` from `list_projects` and creates the repository at once — confirm first) with `type`, `stack`, `name`, `ci` (`github` unless told) and the same `source` if one was used. Leave `cloud` empty here; use the add-cloud skill if a target was named.
